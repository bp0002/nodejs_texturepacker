...
export function optAtlas(fileName: string, saveName: string, compactFrameName: boolean, firstAnime: string) {
    ...
    return readJson(fileName).then((data: ITexturePackAtlas | ITexturePackAtlas[]) => {
        ...
        if (data instanceof Array) {
            result = [];
            data.forEach((item) => {
                (<ITexturePackAtlasCompact[]>result).push(opt(item, compactFrameName, firstAnime));
            });
        } else {
            result = opt(data, compactFrameName, firstAnime);
        }
...
export function opt(data: ITexturePackAtlas, compactFrameName: boolean = true, firstAnime: string) : ITexturePackAtlasCompact {
    let frameIdx = 1;
    ...
    if ((<any>data).compact) {
        ...
        if (data.animations) {
            ...
            if (firstAnime != undefined && data.animations[firstAnime]) {
                let key = data.animations[firstAnime][0];
                let temp = data.frames[key];
                let first: ITexturePackFrameCompact = [
                    temp.rotated ? 1 : 0,
                    temp.trimmed ? 1 : 0,
                    temp.sourceSize.w, temp.sourceSize.h,
                    temp.spriteSourceSize.x, temp.spriteSourceSize.y, temp.spriteSourceSize.w, temp.spriteSourceSize.h,
